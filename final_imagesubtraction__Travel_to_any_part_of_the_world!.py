...
videopath = input("Enter video path name: ")
imagepath = input("Enter new background image path name: ")
...
if os.path.exists(videopath):
    if os.path.exists(imagepath):
        ...
        nframe = int(stream.get(cv2.CAP_PROP_FRAME_COUNT))
        width = int(stream.get(3))
        height = int(stream.get(4))

        #converting the image to grayscale
        bgimg = cv2.imread(imagepath)
        ...
        while True:
            ...
            threshold, mask = cv2.threshold(dif_frame, threshold, 255,cv2.THRESH_BINARY) 
